...
fig_5 = plt.figure(figsize=(fig_width_in, fig_height_in), layout=None)
...
div = Divider(fig_5, (0, 0, 1, 1), h, v, aspect=False)
...
#     line = plt.Line2D([hx, hx], [0, 1], lw=0.5, color='grey', linestyle='--', transform=fig_5.transFigure, zorder=100)
#     fig_5.add_artist(line)
# # Draw horizontal grid lines (row boundaries)
# for vy in v_edge_positions:
#     line = plt.Line2D([0, 1], [vy, vy], lw=0.5, color='grey', linestyle='--', transform=fig_5.transFigure, zorder=100)
#     fig_5.add_artist(line)
#######################################################################
#################### end: show grid via grey lines ####################
#######################################################################

ax_amplitude = fig_5.add_axes(
    div.get_position(), axes_locator=div.new_locator(nx=5, ny=5))
ax_theta_uncertainty = fig_5.add_axes(
    div.get_position(), axes_locator=div.new_locator(nx=5, ny=2))
ax_theta_reconstructed = fig_5.add_axes(
    div.get_position(), axes_locator=div.new_locator(nx=2, ny=5))
ax_theta_bias = fig_5.add_axes(
...
thetas_rec_ell_num      = num_eval_theta_scan_geom['thetas_rec_ell']
thetas_rec_ell_std_num  = num_eval_theta_scan_geom['thetas_rec_ell_std']

# check
thetas_set_geom              = num_eval_theta_scan_geom['thetas_set']
print("thetas_set_geom", np.sum(thetas_set_geom-thetas_set))
...
# Results of geometric ellipse fits
exp_coarse_eval_geom = np.load('../exp_eval/exp_coarse_eval_res_S_sum_geo_ell.npz')

thetas_bias_ell_exp_coarse      = exp_coarse_eval_geom['theta_bias_ell']
thetas_rec_ell_std_exp_coarse   = exp_coarse_eval_geom['theta_ell_std']

a_calib_coarse_geom                    = exp_coarse_eval_geom['a_calib']
a_calib_coarse_unct_geom               = exp_coarse_eval_geom['a_calib_unct']

# check
thetas_calibrated_exp_coarse_geom    = exp_coarse_eval_geom['theta_calibrated_ell']
print("thetas_calibrated_exp_coarse_geom", np.sum(thetas_calibrated_exp_coarse_geom-thetas_calibrated_exp_coarse))

print("a_calib_coarse_geom", np.sum(a_calib_coarse_geom-a_calib))

print("a_calib_coarse_unct_geom", np.sum(a_calib_coarse_unct_geom-a_calib_unct))
...
### EXP DATA ###
exp_fine_eval                   = np.load(f'../exp_eval/exp_fine_eval_res_S_sum_PEAC.npz')
...
# Results of geometric ellipse fits
exp_fine_eval_geom = np.load('../exp_eval/exp_fine_eval_res_S_sum_geo_ell.npz')

thetas_bias_ell_exp_fine      = exp_fine_eval_geom['theta_bias_ell']
thetas_rec_ell_std_exp_fine   = exp_fine_eval_geom['theta_ell_std']

a_calib_fine_geom                    = exp_fine_eval_geom['a_calib']
a_calib_fine_unct_geom               = exp_fine_eval_geom['a_calib_unct']

# check
thetas_calibrated_exp_fine_geom    = exp_fine_eval_geom['theta_calibrated_ell']
print("thetas_calibrated_exp_fine_geom", np.sum(thetas_calibrated_exp_fine_geom-thetas_calibrated_exp_fine))

print("a_calib_fine_geom", np.sum(a_calib_fine_geom-a_calib))

print("a_calib_fine_unct_geom", np.sum(a_calib_fine_unct_geom-a_calib_unct))
...
ax_theta_uncertainty.set_ylabel(r"$\Delta \theta / \pi\times 10^{-3}$")
# ax_theta_uncertainty.grid(True, linewidth=0.6, alpha=0.5)

ax_theta_uncertainty.set_xlim(0.5, 2.5)
ax_theta_uncertainty.set_ylim(0.0, 0.07*1e3)

ax_theta_uncertainty.set_xticks([0.5, 1.0, 1.5, 2.0, 2.5])
ax_theta_uncertainty.set_yticks([0.00*1e3, 0.01*1e3, 0.02*1e3, 0.03*1e3, 0.04*1e3, 0.05*1e3, 0.06*1e3])
...
###############################################################################
# numerical data
ax_theta_uncertainty.plot(thetas_set/np.pi, thetas_rec_ell_std_num*1e3/np.pi,
                          color=colour_ell, linewidth=1)
ax_theta_uncertainty.plot(thetas_set/np.pi, thetas_rec_diff_std_num*1e3/np.pi,
                          color=colour_diff, linewidth=1)
ax_theta_uncertainty.plot(thetas_set/np.pi, thetas_rec_sum_std_num*1e3/np.pi,
...
# ax_theta_uncertainty.plot(
#     thetas_set[wp_index]/np.pi,
#     thetas_rec_sum_std_num[wp_index]/np.pi,
#     marker=marker_cross,
#     ls="",
#     ms=6,
#     mec='black',
...
arrow = Arrow(thetas_set[wp_index]/np.pi,
              7.5+thetas_rec_sum_std_num[wp_index]*1e3/np.pi,
              dx = 0,
              dy =-6.5,
              color='limegreen',
              width=0.04,
              zorder=5,
              clip_on=False)
ax_theta_uncertainty.add_artist(arrow)

# experimental data
hf.plot_line_with_wide_err(ax_theta_uncertainty, thetas_calibrated_exp_coarse/np.pi,
                           thetas_rec_ell_std_exp_coarse*1e3/np.pi, 0.0, 0, colour_ell, ls="")

hf.plot_line_with_wide_err(ax_theta_uncertainty, thetas_calibrated_exp_coarse/np.pi,
                           thetas_rec_diff_std_exp_coarse*1e3/np.pi, 0.0, 0, colour_diff, ls="")

hf.plot_line_with_wide_err(ax_theta_uncertainty, thetas_calibrated_exp_coarse/np.pi,
                           thetas_rec_sum_std_exp_coarse*1e3/np.pi, 0.0, 0, colour_sum, ls="")
...
axins_theta_uncertainty.plot(thetas_set/np.pi, thetas_rec_ell_std_num*1e3/np.pi,
                                color=colour_ell, linewidth=1)
# axins_theta_uncertainty.plot(thetas_set/np.pi, thetas_rec_diff_std_num,
#                                 color=colour_diff, linewidth=1)
axins_theta_uncertainty.plot(thetas_set/np.pi, thetas_rec_sum_std_num*1e3/np.pi,
                                color=colour_sum, linewidth=1)

hf.plot_line_with_wide_err(axins_theta_uncertainty, thetas_calibrated_exp_fine/np.pi, thetas_rec_ell_std_exp_fine*1e3/np.pi,
...
hf.plot_line_with_wide_err(axins_theta_uncertainty, thetas_calibrated_exp_fine/np.pi,
                           thetas_rec_sum_std_exp_fine*1e3/np.pi, 0, 0, colour_sum, marker="|", ls="")


# limit inset to the highlighted region
x1_inset_theta_unct, x2_inset_theta_unct = 0.84, 1.14   # theta_set / pi
y1_inset_theta_unct, y2_inset_theta_unct = 0, 0.03*1e3  # theta_reconstructed / pi
...
ax_theta_bias.plot(thetas_set/np.pi, (thetas_rec_diff_num-thetas_set)*1e3/np.pi,
                   color=colour_diff, linewidth=1)
ax_theta_bias.fill_between(thetas_set/np.pi,
                           (thetas_rec_diff_num-thetas_set -
                           1*thetas_rec_diff_std_num)*1e3/np.pi,
                           (thetas_rec_diff_num-thetas_set +
                           1*thetas_rec_diff_std_num)*1e3/np.pi,
                           color=colour_diff, alpha=0.3)

ax_theta_bias.plot(thetas_set/np.pi, (thetas_rec_ell_num-thetas_set)*1e3/np.pi,
                   color=colour_ell, linewidth=1)
ax_theta_bias.fill_between(thetas_set/np.pi,
                           (thetas_rec_ell_num-thetas_set - 1*thetas_rec_ell_std_num)*1e3/np.pi,
                           (thetas_rec_ell_num-thetas_set + 1*thetas_rec_ell_std_num)*1e3/np.pi,
                           color=colour_ell, alpha=0.3)

ax_theta_bias.plot(thetas_set/np.pi, (thetas_rec_sum_num-thetas_set)*1e3/np.pi,
                   color=colour_sum, linewidth=1)
ax_theta_bias.fill_between(thetas_set/np.pi,
                           (thetas_rec_sum_num-thetas_set - 1*thetas_rec_sum_std_num)*1e3/np.pi,
                           (thetas_rec_sum_num-thetas_set + 1*thetas_rec_sum_std_num)*1e3/np.pi,
