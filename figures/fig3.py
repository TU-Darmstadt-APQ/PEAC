...
amplitude_results = np.load('../exp_eval/exp_coarse_fits_res_A_sum_PEAC.npy')
baseline_results = np.load('../exp_eval/exp_coarse_fits_res_S_sum_PEAC.npz')['results_histogram'][:,:,2].mean()
...
for sign in [1, -1]:
    ax_1.plot(
        times_fine,
        sign * Amp_sum(times_fine * 1e-3, *amplitude_results.mean(axis=0)) + baseline_results ,
        color='w', lw=line_width
    )

# Add vertical markers for example time points
ax_1.vlines([1, 1.4, 1.7, 2.5], -np.sqrt(2), np.sqrt(2), ls='--', lw=0.75, color='k')
...
for i, ind in enumerate(indices):
    ...
    axs[2,i].text(locs[i],1*limit/1.1,labels[i],c='k',ha=aligns[i],va='top')
    if i>0:
        axs[2,i].set_yticks([])
...
fig.text(0.13,0.705,r'$a$',va='top',ha='left',c='k')
fig.text(0.3,0.705,r'$b$',va='top',ha='left',c='k')
fig.text(0.42,0.705,r'$c$',va='top',ha='left',c='k')
fig.text(0.76,0.705,r'$d$',va='top',ha='left',c='k')
