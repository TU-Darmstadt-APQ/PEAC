...
####################################################
### Find minimum in standard deveations and fit linear trend###
####################################################
...
mesh_left = ax_abs_bias.pcolormesh(
        thetas_set/np.pi,
        sigmas_set,
        theta_bias_sum_sigmas_abs/np.pi,
        cmap="jet",
        shading="auto",
        # norm="log"
        rasterized=True,
...
cbar_left.set_label(r"$|\theta_{\text{bias, sum}}|/\pi$", labelpad=3)
...
mesh_right = ax_std_bias.pcolormesh(
        thetas_set/np.pi,
        sigmas_set,
        thetas_rec_sum_sigmas_std / np.pi,
        cmap="jet",
        shading="auto",
        # norm="log",
        rasterized=True,
...
cbar_right.set_label(r"$\Delta \theta_\text{sum} / \pi$", labelpad=3)
