...
## MLE
ax_theta_bias.plot(thetas_set_MLE[start:stop]/np.pi,
                   (thetas_rec_sum_num_MLE[start:stop]-thetas_set_MLE[start:stop])/np.pi*1e3,
                   color="tab:green",
                   linewidth=1,
                   label=r'MLE', zorder=2)

ax_theta_bias.fill_between(thetas_set_MLE[start:stop]/np.pi,
                           (thetas_rec_sum_num_MLE[start:stop]-thetas_set_MLE[start:stop] - 1*thetas_rec_sum_std_num_MLE[start:stop])/np.pi*1e3,
                           #
                           (thetas_rec_sum_num_MLE[start:stop]-thetas_set_MLE[start:stop] + 1*thetas_rec_sum_std_num_MLE[start:stop])/np.pi*1e3,
                           color="tab:green", alpha=0.3)

## LSQ
ax_theta_bias.plot(thetas_set_PEAC[start:stop]/np.pi,
                   (thetas_rec_sum_num_PEAC[start:stop]-thetas_set_PEAC[start:stop])/np.pi*1e3,
                   color=colour_sum,
                   linewidth=1,
                   label=r'LSQ')

ax_theta_bias.fill_between(thetas_set_PEAC[start:stop]/np.pi,
                           (thetas_rec_sum_num_PEAC[start:stop]-thetas_set_PEAC[start:stop] - 1*thetas_rec_sum_std_num_PEAC[start:stop])/np.pi*1e3,
                           #
                           (thetas_rec_sum_num_PEAC[start:stop]-thetas_set_PEAC[start:stop] + 1*thetas_rec_sum_std_num_PEAC[start:stop])/np.pi*1e3,
                           color=colour_sum, alpha=0.3)

## ellipses
ax_theta_bias.plot(thetas_set_geom[start:stop]/np.pi,
                   (thetas_rec_ell_num_geom[start:stop]-thetas_set_geom[start:stop])/np.pi*1e3,
                   color=colour_ell,
                   linewidth=1,
                   label=r'ellipses', zorder=-1)
...
# ax_theta_bias.set_ylim(-0.1/np.pi, 0.1/np.pi)
ax_theta_bias.legend(loc='upper right',
                     borderpad = 0.2,
                     handlelength = 1,
...
ax_theta_uncert.plot(thetas_set_MLE[start:stop]/np.pi,
                     thetas_rec_sum_std_num_MLE[start:stop]/np.pi*1e3, color="tab:green",
                     linewidth=1, label=r'MLE')

ax_theta_uncert.plot(thetas_set_PEAC[start:stop]/np.pi,
                     thetas_rec_sum_std_num_PEAC[start:stop]/np.pi*1e3, color=colour_sum,
                    linewidth=1, label=r'PEAC')

ax_theta_uncert.plot(thetas_set_geom[start:stop]/np.pi,
                     thetas_rec_ell_std_num_geom[start:stop]/np.pi*1e3, color=colour_ell,
